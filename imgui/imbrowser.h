...
inline void ImGui::FileBrowser::Display()
{
    ...
    {
        if (!BeginPopup(openLabel_.c_str(), ImGuiWindowFlags_NoSavedSettings))
        {
            return;
        }
    }
    else if (!BeginPopupModal(openLabel_.c_str(), nullptr, ImGuiWindowFlags_NoSavedSettings |
    ...
    if (newDirNameBuf_)
    {
        ...
        if (BeginPopup(openNewDirLabel_.c_str(), ImGuiWindowFlags_NoSavedSettings))
        {
